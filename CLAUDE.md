...
│   ├── handlers.py          # Telegram handlers: photo, document, text; save_extraction() (shared auto-commit helper, also used by backend/routers/chat.py's upload endpoint)
...
│       ├── categories.py               # POST /api/categories — create a custom transaction category
│       └── chat.py                     # POST /api/chat (web-dashboard Q&A, via bot/finance_agent.py::answer_question(channel="web")) + POST /api/chat/upload (receipt/screenshot upload, auto-commits via bot/handlers.py::save_extraction)
...
- `spending.py` — `GET /api/transactions`, `GET /api/transactions/summary` (wraps `scheduler/report_builder.summarize_transactions()`), `POST /api/transactions` (validates `category` against `get_categories_for_user(user_id)`, raising 422 if it's not in the user's built-in+custom list), `PATCH /api/transactions/{id}` (category/description only, same restriction as the dashboard's editable table).
...
- `accounts.py` — `GET /api/accounts` (optional `?type=` comma-separated filter), `POST /api/accounts` (create an account — the dashboard-side equivalent of the bot's `/newaccount`, surfaced in the Settings page), `GET /api/accounts/balances` (wraps `utils/balances.py::compute_account_balances()` — the `/balance` bot command's math).
- `categories.py` — `POST /api/categories` (body: `{name}`) calls `db.supabase.create_custom_category(user_id, name)`, surfaced in the Settings page. Unique per `(user_id, name)` — a duplicate name for the same user 500s on the DB constraint (caught client-side as "may already exist").
- `chat.py` — `POST /api/chat` (body: `{message}`, response: `{reply}`) calls `bot/finance_agent.py::answer_question(user_id, message, user_id, channel="web")` in a threadpool (it's blocking DeepSeek + Supabase I/O, same reason as `/api/refresh-prices`/`/api/dividend-forecast`). This is the web-dashboard equivalent of the Telegram bot's free-text Q&A path — same tools, same tenant scoping, no separate agent implementation. See "Finance Q&A Agent" below for what `channel` changes. Also `POST /api/chat/upload` (multipart: `file`, `account_id`) — the web equivalent of the bot's `handle_photo`/`handle_document`: extracts via `bot/extractor.py`'s `extract_from_image`/`extract_from_pdf_images` (chosen by `file.content_type`, run in a threadpool, `categories` from `get_categories_for_user`) and auto-commits via `bot/handlers.py::save_extraction` (the exact same function the bot uses), returning the saved row ids so the frontend can offer an undo. New transaction `source` values `web_image`/`web_pdf` distinguish this path in the data.
...
- **Settings page** (`src/pages/SettingsPage.tsx`): "Link Telegram" — calls `POST /api/telegram-link` (`useGenerateTelegramLinkCode` in `src/hooks/api.ts`), displays the code and `/link <code>` instructions. Since generating a code doesn't itself complete the link (the user still has to message the bot), a "refresh status" action re-calls `refreshMe()` rather than assuming success. Also "Your Accounts" (list + inline create form, `useCreateAccount`, invalidates `["accounts"]`) and "Transaction Categories" (list from `useMeta()` + inline create form, `useCreateCategory`, invalidates `["meta"]`) — both the dashboard-side equivalent of the bot's `/newaccount` and the extraction-aware custom-category feature (see "Categories" above).
...
- **Chat page** (`src/pages/ChatPage.tsx`): its own routed nav item (`/chat`), not a floating widget — mounted once as a normal page inside `Layout.tsx`'s `<Outlet />` like every other page. Sends each message via `useSendChatMessage` (`src/hooks/api.ts`) to `POST /api/chat`. Message list is local component state only — no persistence, resets on reload — since the multi-turn context that exists comes from `bot/finance_agent.py`'s own in-memory `chat_history` (see "Finance Q&A Agent"), not from anything the frontend stores. Also hosts the **web upload feature**: an attach button next to the input opens a file picker (`accept="image/*,application/pdf"`); the selected file plus an account (auto-picked if the user has only one, otherwise a `<select>`) is posted via `useUploadChatFile` (`FormData`, `api.upload()` in `src/api/client.ts` — skips the JSON `Content-Type` header so the browser can set the multipart boundary) to `POST /api/chat/upload`. On success the reply bubble shows the saved rows plus an **Undo** button (`useUndoUpload`) that deletes each returned id via the existing per-row `DELETE /api/transactions/{id}`/`DELETE /api/portfolio-events/{id}` endpoints and re-invalidates every query key the write could have affected (`transactions`, `expense-summary`, `portfolio-events`, `holdings`, `balances`, `snapshots`) so other pages reflect it without a manual refresh; the bubble then shows "↩️ Undone" instead of removing itself from the transcript.
