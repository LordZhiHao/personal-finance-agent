...
│   ├── dividends.py         # compute_dividend_total(): FX-converted sum of DIVIDEND-action portfolio_events over a date range (for GET /api/dividends/summary — the Investments page's Year to Date Dividends KPI card)
...
```
users              id, email, password_hash, telegram_chat_id, notify_email, main_currency, theme, hidden_dashboard_sections, onboarding_completed_at, created_at
...
- `custom_categories.classification` (added `migrations/0015_category_classification.sql`, `text not null default 'expense'`, one of `expense | income | transfer | investment`) — see "Categories" below for what each value means and where it's enforced. `db.supabase.get_category_classifications_for_user(user_id)` merges this with the built-in mapping (`utils/constants.py::BUILTIN_CATEGORY_CLASSIFICATIONS`) into one `{category: classification}` dict — the one shared lookup every spend/budget/subscription aggregation uses
...
- `users.hidden_dashboard_sections` (added `migrations/0016_user_dashboard_sections.sql`, `jsonb not null default '[]'::jsonb`) is a per-user list of chart/section keys hidden from the Spending/Investments pages (Settings' "Customize Dashboard" card), e.g. `["spending.byCategory", "investments.trades"]`. Stores the **hidden** set rather than an enabled allowlist specifically so a newly-shipped chart defaults to visible for every existing user with zero backfill. Keys are namespaced `"<view>.<tabId>"`, built by `frontend/src/lib/dashboardSections.ts::sectionKey()` from the same tab-id strings already used by `SpendingPage.tsx`'s `SpendingTab`/`SPENDING_TABS` and `InvestmentsPage.tsx`'s `InvestmentsTab`/`INVESTMENTS_TABS`. `backend/schemas.py::MeUpdate`'s validator only strips/dedupes/caps the list (50 entries) — it does **not** whitelist keys server-side, since section keys are a frontend-only concept and a strict whitelist would force a backend redeploy for every future chart addition/rename; stale keys left over from a removed chart are harmless no-ops. Each view's KPI summary row (`"spending.summary"`/`"investments.netWorth"`) is permanently pinned — not offered as a checkbox — partly because Investments' summary row is the only place the "Refresh Prices" button lives.
...
Users can add, rename, and delete their own categories on top of this list from the frontend's Settings page (`POST`/`PATCH`/`DELETE /api/categories` → `db.supabase.create_custom_category`/`update_custom_category`/`delete_custom_category`, stored in `custom_categories`; `GET /api/categories` returns just the user's own `{id, name, classification}` rows for that management UI). **The full valid category list for a given user is always `db.supabase.get_categories_for_user(user_id)`** (`CATEGORIES + get_custom_categories(user_id)`), not the bare `CATEGORIES` constant — this is what `GET /api/meta` returns, what the Gemini/DeepSeek extraction prompts are built from per-request (see "LLM Extraction" below), and what `POST /api/transactions` validates a submitted category against. A category one user creates is invisible to every other user (unique per `(user_id, name)`). Built-in categories (`CATEGORIES`) can't be renamed or deleted — only a user's own custom ones.

**Classification** — every category also has a classification, `expense | income | transfer | investment` (`utils/constants.py::CLASSIFICATIONS`), controlling whether it counts as spending. Built-ins get a fixed mapping in `utils/constants.py::BUILTIN_CATEGORY_CLASSIFICATIONS` (`Salary → income`, `Investment → investment`, `Transfer → transfer`; everything else, including `Other`, defaults to `expense`); a custom category's own `classification` column defaults to `expense` but can be set to any of the four at creation (`POST /api/categories`/`create_category` chat tool) or changed later (`PATCH /api/categories/{id}`/`rename_category`... — see `bot/finance_agent.py`). `db.supabase.get_category_classifications_for_user(user_id)` is the merged `{category: classification}` lookup every aggregation below uses. Only `expense`-classified, negative-amount transactions count toward `scheduler/report_builder.py::summarize_transactions()`'s `expenses`/`by_category`, `month_comparison()`, `budget_status()`, and `utils/subscriptions.py::detect_recurring_charges()` — an `investment`-classified transaction (e.g. a brokerage top-up) is broken out into a separate `invested` total instead (surfaced as its own "Invested" stat on `SpendingPage`, its own line in `/expense`/the weekly report, and its own field in the finance agent's `get_spending_summary` tool result), and `income`/`transfer`-classified negative rows count toward neither. `POST /api/budgets`/the `create_budget` chat tool reject creating a budget against a non-`expense` category.
...
- `spending.py` — `GET /api/transactions` (`?currency=`, default `"SGD"` — adds a `converted_amount` field per row via `utils/fx.convert`, alongside the untouched native `amount`/`currency`, so pages that sum across transactions of different currencies — see the Spending page in "Currency & Theme Preferences" above — aggregate correctly), `GET /api/transactions/summary` (wraps `scheduler/report_builder.summarize_transactions()`, itself still unconverted — see that same section's "Known, deliberately out-of-scope gap"), `POST /api/transactions` (validates `category` against `get_categories_for_user(user_id)`, raising 422 if it's not in the user's built-in+custom list), `PATCH /api/transactions/{id}` (description/amount/category/account_id — the dashboard's editable table only ever sends description/category, so its restriction is unchanged; `amount` keeps the negative=expense sign convention and is sent as the raw signed value; moving a transaction to a different account validates the new `account_id` is also owned by the caller, same convention as `update_portfolio_event`), `GET /api/transactions/{id}/receipt` (ownership-checked via `db.supabase.get_transaction_receipt()`, 404 if no `receipt_id`; otherwise returns a short-lived signed Supabase Storage URL via `db.supabase.create_signed_receipt_url()` — never a raw storage path or key, since the bucket is private and the frontend never holds a Supabase key).
- `investments.py` — `GET /api/snapshots` (adds a `converted_value` field per row via `utils/fx.convert`, since the frontend has no FX calls of its own — pass `?currency=`), `GET`/`POST /api/portfolio-events` (`GET` also accepts an optional `?currency=`, same `converted_value`-per-row enrichment as `/snapshots` — `converted_value = quantity * price` converted to that currency — used by the Dividends by Currency donut below; omitting it leaves every other caller, e.g. the trade history table/edit dialogs/Dividend Calendar, unchanged), `GET /api/holdings` (wraps `utils/portfolio.py::compute_holdings_summary()` — the `/portfolio` bot command's math, first surfaced outside Telegram here; each holding includes `price`, the latest per-unit market price, alongside `avg_cost`/`market_value`), `GET /api/dividends/summary` (`?currency=&start_date=&end_date=`, wraps `utils/dividends.py::compute_dividend_total()` — sums `DIVIDEND`-action `portfolio_events` over the given range, FX-converting each row to `currency`; powers the Investments page's Year to Date Dividends KPI card).
...
- **Settings page** (`src/pages/SettingsPage.tsx`): "Link Telegram" — calls `POST /api/telegram-link` (`useGenerateTelegramLinkCode` in `src/hooks/api.ts`), displays the code and `/link <code>` instructions. Since generating a code doesn't itself complete the link (the user still has to message the bot), a "refresh status" action re-calls `refreshMe()` rather than assuming success. Also:
  ...
  - "Customize Dashboard" (`CustomizeDashboardCard`) — checkbox group per view (Spending/Investments), sourced from the shared `src/lib/dashboardSections.ts::DASHBOARD_SECTIONS` metadata (each view's pinned KPI row excluded from the list), same dirty-draft-then-Save pattern as `MainCurrencyCard`/`ThemeCard` but with array-based dirty-checking; saves via `useUpdateHiddenDashboardSections()`. See the `users.hidden_dashboard_sections` bullet under "Database Schema" above and `SpendingPage.tsx`/`InvestmentsPage.tsx` below for how each page reads it back.
...
- **Pages** (`src/pages/`): `SpendingPage` and `InvestmentsPage` mirror the legacy Streamlit pages' charts; `PortfolioPage` (holdings/avg-cost/unrealized P&L) and `BalancesPage` (unified cash+brokerage balances) are new — they surface `/api/holdings` and `/api/accounts/balances`, which the Streamlit dashboard never called even though the underlying bot commands (`/portfolio`, `/balance`) already existed. `ChatPage` (see below) is its own nav item rather than a floating widget. `SpendingPage`/`InvestmentsPage` both build each chart section as a local JSX const, reused by `SwipeableSections` for two layouts: a mobile tab carousel (`panels`) and a desktop stack (`desktopContent`). Each page filters its own tab list against `useAuth().hiddenDashboardSections` before passing it to `MobileSectionTabs`/`SwipeableSections` (hiding a section from `panels` is then automatic, since `SwipeableSections` only renders panels for tabs present in the given `tabs` array), and swaps its desktop 8/4 pair `<div>`s for the shared `src/components/SectionPairRow.tsx`, which collapses a pair to full-width when only one side is visible (or renders nothing when both are hidden) rather than leaving an empty grid gap. The 8/4 split is the default, overridable per-pair via `leftSpan`/`rightSpan` props (out of 12, values looked up against a static Tailwind class map so purge doesn't strip them) — `InvestmentsPage.tsx`'s Top Holdings/Dividend Calendar and Dividends by Currency/Upcoming Dividends pairs both use `leftSpan={7} rightSpan={5}` so the narrower, right-hand card gets a bit more room than the default 4/12.
- **Charts** (`src/components/charts/`): Recharts, colored via `src/lib/palette.ts`'s fixed categorical order (`CATEGORICAL`) — category colors specifically go through `colorForCategory()`, which maps the 8 expense-relevant categories (`EXPENSE_CATEGORY_COLOR_ORDER`) to the 8 validated hues and folds everything else (income-only categories, "Other") to a neutral rather than wrapping/reusing a hue — a wrapped 9th+ category previously collided with an earlier one (e.g. "Transfer" and "Transport" rendered identically). Two chart-only additions beyond the Streamlit set: `SpendingHeatmap` (daily spend calendar) and `MonthComparisonBarChart` (this month vs previous vs same month last year per category), both pure client-side aggregations over already-fetched transactions (`src/lib/dates.ts`) — no dedicated backend endpoint. `HoldingsTable` (shared by `PortfolioPage` and `InvestmentsPage`'s "Positions" table) includes a "Latest Price" column (the `price` field `compute_holdings_summary` already fetched internally to compute `market_value`, now also returned). `DividendsByCurrencyDonut` (`InvestmentsPage`'s "Dividends by Currency" chart, modeled directly on `SpendByCategoryDonut`) groups `DIVIDEND`-action `portfolio_events` by their own `currency`, but sizes each slice from the **FX-converted** total (`converted_value`, summed per currency group) rather than the raw native `quantity * price` sum — otherwise a MYR total and an SGD total would render as proportionally comparable slices despite representing very different real value. It has its own Year/Month `TabToggle` (year picks from a `<Select>` of distinct years seen in the event history, month uses the same `‹ MMMM yyyy ›` stepper as the spend donut) driven off an always-unbounded `usePortfolioEvents(undefined, undefined, displayCurrency)` fetch (the `displayCurrency`/`main_currency` arg is what triggers the backend's per-row `converted_value` enrichment — see the `investments.py` bullet under "Backend API" above), independent of the page's `FilterBar` date range, so any year in history is browsable regardless of the active filter. Clicking a slice opens the same `Overlay`+`Table` drill-down pattern as `SpendByCategoryDonut`, listing that currency's dividend rows for the selected period — the drill-down's "Total received" figure and each row's `Amount` column stay in the slice's own **native** currency (not converted), since those are literal per-transaction amounts; only the pie's proportions and its tooltip are converted. `DividendCalendar` (`InvestmentsPage`'s "Dividend Calendar" chart) marks days with a dot when a `DIVIDEND`-action event falls on them; a marked day is clickable and opens an `Overlay`+`Table` popup listing that day's dividend events (ticker/quantity/price/amount, all native currency) — unmarked days are inert.
